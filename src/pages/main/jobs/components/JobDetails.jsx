...
const Chip = ({ children, color = "default" }) => (
    <span className={`text-[10px] px-2 py-0.5 rounded-full border ${color === 'green' ? 'bg-emerald-100 text-emerald-700' : 'bg-yellow-100 text-yellow-700'}`}>{children}</span>
);

const Tabs = ({ active, onChange }) => {
    const items = ["Overview", "Detail", "Company"];
    return (
        <div className="flex gap-2 bg-white rounded-full p-1 w-max">
            {items.map((t) => (
                <button
                    key={t}
                    onClick={() => onChange(t)}
                    className={`px-4 py-1.5 rounded-full text-sm ${active === t ? "bg-sky-500 text-white" : "text-slate-600"}`}
                >
                    {t}
                </button>
            ))}
    ...
    return (
        <div className="space-y-6">
            <div className="bg-white rounded-2xl border p-6">
                <div className="text-xl font-semibold text-center">{job.title}</div>
                        ...
                        <div className="bg-white rounded-xl border p-4">
                            ...
                            {selection.resumeId ? (
                                <div className="flex items-center justify-between border rounded-md p-3 text-sm">
                                    <div>Selected: {state.resumes.find((r) => r.id === selection.resumeId)?.name}</div>
                                    <div className="text-slate-400">Change from left</div>
                                </div>
                            ) : (
